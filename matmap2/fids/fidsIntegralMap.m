...
if iscell(startframe),
	...
	for p=1:length(startframe),
		if length(startframe{p}) == 1,
            ...
            if length(startframe{p} ~= numleads), error('The local fiducials vector should have the same number of entries as the number of leads\n'); end
...
if length(startframe) == prod(size(startframe)),
...
if iscell(endframe),
	...
	for p=1:length(endframe),
		if length(endframe{p}) == 1,
            ...
            if length(endframe{p} ~= numleads), error('The local fiducials vector should have the same number of entries as the number of leads\n'); end
...
if length(endframe) == prod(size(endframe)),
...
for p = 1:length(TSindices),
    if isnumeric(TSindices),
        ...
        for q=1:numleads,
            map(q,p) = sum(TS{TSindices(p)}.potvals(q,startframe(q,p):endframe(q,p)));
