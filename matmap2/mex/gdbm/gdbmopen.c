...
/* INCLUDE FUNCTIONS FOR PATCHING THE CODE IN THIS FILE */
#include "gdbmpatch.h"
     ...
     void (*fatal_func) ();
{
  ...
  my_int64_t       file_pos;		/* Used with seeks. */
  ...
  if (file_stat.st_size == 0)
      ...
      if (dbf->header == NULL)
	{
      ...
      dbf->header->dir_size = 8 * sizeof (my_int64_t);
      ...
      if (dbf->header->dir_size != dbf->header->block_size)
	{
      ...
      dbf->dir = (my_int64_t *) malloc (dbf->header->dir_size);
      if (dbf->dir == NULL)
	{
	  gdbm_close (dbf);
	  gdbm_errno = GDBM_MALLOC_ERROR;
      ...
      if (dbf->bucket == NULL)
	{
      ...
      for (index = 0; index < dbf->header->dir_size / sizeof (my_int64_t); index++)
...
/* PATCH: replacement of write() => patch_writeheader()*/
      num_bytes = patch_writeheader (dbf->desc, dbf->header, dbf->header->block_size);
/* END PATCH */
...
/* PATCH: replacement of write() => writedir() */
      num_bytes = patch_writedir (dbf->desc, dbf->dir, dbf->header->dir_size);
/* END PATCH */	  
...
/* PATCH: replacement of write() => patch_writehashbucket() */
      num_bytes = patch_writehashbucket (dbf->desc, dbf->bucket, dbf->header->bucket_size);
/* END PATCH */	  
...
/* PATCH: replacement of read() => patch_readheader() */      
	  num_bytes = patch_readheader (dbf->desc, &partial_header, sizeof (gdbm_file_header));
/* END PATCH */	  
...
/* PATCH: replacement of read() => patch_readavailtable() */
      num_bytes = patch_readavailtable (dbf->desc, &dbf->header->avail.av_table[1],
			dbf->header->block_size-sizeof (gdbm_file_header));
/* END PATCH */			
...
/* PATCH: replacement of read() => patch_readdir() */
      num_bytes = patch_readdir (dbf->desc, dbf->dir, dbf->header->dir_size);
/* END PATCH */
      if (num_bytes != dbf->header->dir_size)
	{
	  gdbm_close (dbf);
