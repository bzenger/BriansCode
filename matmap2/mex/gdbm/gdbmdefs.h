...
/* PATCH: inserted patch here to conform to the alignment on the SGI machines */
  	int   dummy;		
/* END PATCH */	
	my_int64_t  av_adr;		/* The file address of the available block. */
      } avail_elem;
...
typedef struct {
	...
	my_int64_t next_block;	/* The file address of the next avail block. */
...
typedef struct {
	...
	my_int64_t dir;	     /* File address of hash directory table. */
	...
	my_int64_t next_block;    /* The next unallocated block address. */
...
typedef struct {
	...
	my_int64_t data_pointer;	/* The file address of the key record. The
...
/* PATCH: Align the data on a 64bits boundary as is done on the SGI systems */
  	int   dummy;		   
/* END PATCH */	
